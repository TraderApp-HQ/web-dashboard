import type { OpenTrade, Orders, TradeHistory } from "~/lib/types";
  ...
  const tableBody: ITBody = {
    tBodyRows: openTrades.map((trades) => ({
      tBodyColumns: [
        {
          displayItem: renderDisplayItem({
            itemText: { text: `${trades.asset.name} ${trades.pair}`, style: "text-base font-normal" },
        ...
        { displayItem: `${trades.price} USDT` },
        {
          displayItem: renderDisplayItem({
            ...
            itemSubText: {
              text: `${trades.percent}%`,
              style: parseFloat(trades.percent!) < 0 ? "text-rose-700" : "text-emerald-700",
            },
          }),
        },
        {
      ...
      actions: [
        ...
        {
          label: "Manage Trade",
          url: `${trades.id}/${ROUTES.portfolio.manageTrade}`,
        },
      ],
    })),
  };
...
export function tradeHistoryDataTableSelector(openTrades: TradeHistory[]) {
...
export function tradeHistoryDataTableMobileSelector(openTrades: TradeHistory[]) {
  const dataMobile: ITableMobile[] = openTrades.map((trades) => ({
    tHead: {
      displayItemTitle: renderDisplayItem({
        itemText: { text: `${trades.type}`, style: "text-base font-normal" },
