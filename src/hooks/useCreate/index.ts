...
interface MutationOptions<T, Variables = void> {
  ...
  onSettled?: (data: T | undefined, error: Error | null, variables: Variables, context: unknown) => void;
}

type SafeMutationResult<T, Variables = void> = Omit<UseMutationResult<T, Error, Variables, unknown>, "error"> & {
  error: Error | null;
};
...
export const useCreate = <T, Variables = void>({
  ...
  return {
    ...mutation,
    error: mutation.error instanceof Error ? mutation.error : new Error("An unknown error occurred"),
  };
};
