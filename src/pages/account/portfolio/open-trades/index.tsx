...
import { openTradesDataTableSelector, openTradesDataTableMobileSelector } from "~/selectors/portfolio";
      ...
      <Card className="mt-7 lg:w-3/4">
        <div className="grid grid-cols-2 grid-rows-2 lg:flex lg:justify-between">
          <div className="flex-col justify-center items-start gap-0.5 py-3">
            <h3 className="text-neutral-700 text-sm font-normal leading-tight">Total number of trade</h3>
            <div className="justify-center items-center gap-9 inline-flex">
              <p className="text-neutral-700 text-base font-bold">{data.totalActiveSignal}</p>
            </div>
          </div>
          ...
          <div className="flex-col justify-center items-start gap-0.5 py-3">
            ...
            <div className="justify-center items-center gap-9 inline-flex">
              <p className="text-neutral-700 text-base font-bold">{data.totalCapital}.00 USD</p>
            </div>
          </div>
            ...
            <div className="flex-col justify-center items-start gap-2 ml-0 sm:ml-3">
              ...
              <div className="justify-start items-center gap-12 inline-flex">
                <div className="justify-start items-center gap-2 flex">
                  <img src={data.bestSignal?.image} alt={data.bestSignal?.name} className="w-6 h-6 relative" />
                  <p className="text-slate-900 text-xs font-semibold leading-none">{data.bestSignal?.name}</p>
                </div>
                <div className="justify-start items-center flex">
                  <div className="w-4 h-4 relative origin-top-left -rotate-180" />
                  <p className="text-emerald-700 text-sm font-normal">{data.bestSignal.percentage}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="py-3">
            ...
            <div className="w-12 h-px origin-top-left rotate-90 border border-stone-300 border-opacity-20 sm:block hidden" />
            <div className="flex-col justify-center items-start gap-2 ml-0 sm:ml-3">
              <h3 className="text-neutral-700 text-sm font-semibold leading-tight">Worst performer</h3>
              <div className="justify-start items-center gap-12 inline-flex">
                <div className="justify-start items-center gap-2 flex">
                  <img src={data.worseSignal?.image} alt={data.worseSignal?.name} className="w-6 h-6 relative" />
                  <p className="text-slate-900 text-xs font-semibold leading-none">{data.worseSignal?.name}</p>
                </div>
                <div className="justify-start items-center flex">
                  <div className="w-4 h-4 relative origin-top-left -rotate-180" />
                  <p className="text-red-700 text-sm font-normal">{data.worseSignal.percentage}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="mt-2 mb-8">
          <div className="flex justify-between pb-4">
            ...
            <h2 className="text-blue-800 text-sm font-bold cursor-pointer" onClick={onOpenCloseTrade}>
              Close Trade
            </h2>
          </div>
    ...
    <>
      <Modal openModal={openModal} width="md:w-[539px]" onClose={onCancel}>
        <div className="wallet-modal-btn-div flex-col px-8 justify-center text-center space-y-4">
          ...
          <h3 className="text-[#666666] text-sm">Closing all trades cancels all open trade.</h3>
      ...
      <ConfirmCloseTradeValidate onConfirm={handleStageTwoContinue} onCancel={onCancel} openModal={openConfirm} />
    </>
  );
}
  ...
  return (
    <Modal openModal={openModal} width="md:w-[539px]" onClose={onCancel}>
      <div className="wallet-modal-btn-div flex-col px-8 justify-center text-center space-y-4">
        ...
        <h3 className="text-[#666666] text-sm">
          Type <span className="text-[#1836B2]">“close all Trades”</span> in the text filed below to confirm
        </h3>
        <input
          type="text"
