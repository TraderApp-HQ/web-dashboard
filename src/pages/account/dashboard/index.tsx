...
import Table, { THead, THeadData, TBody, TBodyRow, TBodyData } from "~/components/AccountLayout/Table";
    ...
    <div>
      <Card className="flex p-5 justify-between mb-5 items-center">
        <div>
          ...
          {userProfile && (
            <div className="pb-9">
              <h3 className="text-[#102477] font-bold text-3xl">Hello {userProfile.firstName} !</h3>
              <h5 className="text-base">Welcome back to your account,</h5>
            </div>
          )}
          <div className="flex justify-start space-x-2" onClick={() => handleShowBalance(!showBalance)}>
      ...
      <Card className="p-5">
        ...
        <div className="lg:flex lg:justify-between grid grid-cols-2 grid-rows-2 gap-y-3">
          <div>
            ...
            <h3 className="text-[#08123B] font-bold text-xl">{signals?.totalActiveSignal}</h3>
          </div>
          <div>
            <h3 className="text-base text-[#414141]">Total Profit</h3>
            <h3 className="text-[#08123B] font-bold text-xl">{signals?.totalCapital}</h3>
          </div>
          <div>
            <h3 className="text-base text-[#414141]">Best performer</h3>
            <h3 className="text-[#08123B] font-bold text-xl">{signals?.bestSignal.amount}</h3>
          </div>
          <div>
            <h3 className="text-base text-[#414141]">Worst performer</h3>
            <h3 className="text-[#08123B] font-bold text-xl">{signals?.worseSignal?.amount}</h3>
          </div>
        </div>
      </Card>
            ...
            <TBodyRow>
              <TBodyData>
                <div className="flex gap-3 items-start justify-start">
                  <img src={item.image} alt={item.shortName} width={"30px"} height={"30px"} />
                  <div className="space-y-1 flex flex-col items-start text-[#1E1E1E] ">
                    <span className="text-base font-semibold">{item.curency}</span>
