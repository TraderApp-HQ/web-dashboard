    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white p-6 rounded-md shadow-md w-full max-w-md">
        ...
        <div className="flex justify-center gap-4 mt-8">
          <button className="px-4 py-2 bg-blue-800 text-white border-radius rounded-md" onClick={handleDisableUser}>
            {!isPending ? (user.status === UserStatus.ACTIVE ? "Remove User" : "Reactivate User") : "... Processing"}
          </button>
          <button className="px-4 py-2 bg-white text-black border rounded-md" onClick={onClose}>
            Cancel Request
          </button>
        </div>
      </div>
