...
export default function CreateSignal() {
  ...
  const isSubmitDisabled = !(asset && timeFrame && signalImage && targetProfits.length && entryPrice && stopLoss);
          ...
          <Form onSubmit={onSubmit} onReset={onReset} method="post">
            <div className="flex flex-col gap-y-4">
              ...
              <div>
                <InputField
                  type="number"
                  props={{ disabled: targetProfits.length > 3, name: "targetProfit" }}
                  ...
                  {targetProfits.map((profit, index) => (
                    <div
                      ...
                      className={`py-1 px-1.5 rounded border justify-center items-center gap-2 flex ${
                        index === 0 ? "border-emerald-700" : "border-stone-300"
                      }`}
                    >
                      <p
                        className={`text-xs font-normal leading-none ${
                          index === 0 ? "text-emerald-700" : "text-zinc-500"
                        }`}
                      >
                        {profit}%
                      </p>
              ...
              <InputField
                ...
                onKeyDown={handleKeyDown}
              />
              <label className="text-slate-900 text-sm font-normal leading-none">Upload signal image</label>
              <UploadButton acceptedFiles={signalImage} getRootProps={getRootProps} getInputProps={getInputProps} />
              <Button
