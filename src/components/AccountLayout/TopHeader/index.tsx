...
const TopHeader: React.FC<ITopHeaderProps> = ({ clientApp }) => {
  ...
  useEffect(() => {
    ...
    if (userData.role.includes(UserRole.ADMIN) || userData.role.includes(UserRole.SUPER_ADMIN)) {
      setIsAdmin(true);
    }
  }, []);
  return (
      ...
      <div>
        {isAdmin && (
          <div className="flex-col justify-start items-start gap-1 inline-flex">
            <span className="text-slate-400 text-sm font-normal leading-tight">Switch Account</span>
            <div className="justify-center items-center gap-1 inline-flex">
              {clientApp === "ADMIN_DASHBOARD" ? (
                <Link className="text-slate-900 text-base font-bold leading-normal" href={`..${LAYOUT_ROUTES.account}`}>
                  User Dashboard
                </Link>
              ) : (
