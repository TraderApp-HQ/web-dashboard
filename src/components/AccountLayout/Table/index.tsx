...
export function TFooterData({ children }: { children: React.ReactNode }) {
  return (
    <td className=" h-10 px-4 py-3">
      <p className="text-cyan-950 text-xs font-light leading-none tracking-wide whitespace-nowrap">{children}</p>
    </td>
  );
}
...
export function TFooter({
  ...
  return (
    <tfoot
      className={clsx("h-10 mt-4 grid px-4 items-center absolute w-full", position === "right" ? "justify-end" : "")}
    >
      <tr>
        {defaultFooter ? (
...
export function TBodyData({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <td className={clsx("h-10 px-6 py-3 border-b border-slate-200", className)}>
      <p className="text-zinc-600 text-sm font-normal leading-none whitespace-nowrap">{children}</p>
    </td>
  );
}
