...
const ProfileDropdown = () => {
  ...
  const { userEmail, userFirstNameInitials, userLastNameInitials, userFullName } = useUserProfileData();
    ...
    <DropdownMenu
      ...
      children={
        <div>
          ...
          <hr className="pb-4" />
          <div className="flex flex-col">
            <DropdownMenuItem type="link" to={LAYOUT_ROUTES.account + ROUTES.settings} className="pl-0">
              <p className="flex cursor-pointer pb-2">
                <UserProfileIcon color="#3C3C3F" />
                <span className="text=[14px] ml-3 text-[#3C3C3F] hover:text-[#1836B2]">View Profile</span>
              </p>
            </DropdownMenuItem>
            <DropdownMenuItem type="button" className="pl-0" onClick={handleLogout}>
              <p className="flex cursor-pointer">
                <LogoutIcon />
                <span className="text=[14px] ml-3 text-[#3C3C3F] hover:text-[#1836B2]">Logout</span>
              </p>
            </DropdownMenuItem>
          </div>
        </div>
