...
import { recentTransactionsDataTableSelector, recentTransactionsDataTableMobileSelector } from "~/selectors/wallets";

export default function RecentTransactions() {
  const transactionsResult = data;

  const { tableHead, tableBody } = recentTransactionsDataTableSelector(transactionsResult?.transactions);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mt-12 font-bold">
        <h4 className="text-slate-900 text-base font-bold leading-7">Recent Transactions</h4>
      </div>
