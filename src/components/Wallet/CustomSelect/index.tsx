...
export default function CustomSelect({
  ...
  return (
    <div ref={ref} className={clsx("relative cursor-pointer", className)}>
      <label className={clsx("text-slate-900 text-sm font-normal leading-none")}>{label}</label>
      <div
        onClick={handleClick}
        className={clsx(
      ...
      >
        <div className="flex ml-2 space-x-2 items-center bg-slate-50">
          {selectedDefault?.image && (
            <img src={selectedDefault?.image} alt={selectedDefault.name} className="w-6 h-6" />
          )}
                ...
                <li key={key} onClick={() => handleSelect(item)}>
                  ...
                  >
                    <div className="inline-flex items-center">
                      {item?.image && <img src={item?.image} alt={item.name} className="w-6 h-6 mr-2" />}
                      {item?.name}
                    </div>
                  </button>
                </li>
