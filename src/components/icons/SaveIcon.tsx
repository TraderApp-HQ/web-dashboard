import React from "react";

export default function SaveIcon() {
	return (
		<svg
			width="40"
			height="40"
			viewBox="0 0 40 40"
			fill="none"
			xmlns="http://www.w3.org/2000/svg"
		>
			<rect width="40" height="40" rx="20" fill="#F0F5FF" />
			<path
				d="M20.8335 11.6665H15.0002C14.5581 11.6665 14.1342 11.8421 13.8217 12.1547C13.5091 12.4672 13.3335 12.8911 13.3335 13.3332V26.6665C13.3335 27.1085 13.5091 27.5325 13.8217 27.845C14.1342 28.1576 14.5581 28.3332 15.0002 28.3332H25.0002C25.4422 28.3332 25.8661 28.1576 26.1787 27.845C26.4912 27.5325 26.6668 27.1085 26.6668 26.6665V17.4998M20.8335 11.6665L26.6668 17.4998M20.8335 11.6665V17.4998H26.6668"
				stroke="#1836B2"
				strokeWidth="1.66667"
				strokeLinecap="round"
				strokeLinejoin="round"
			/>
		</svg>
	);
}
